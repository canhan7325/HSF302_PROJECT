...
public interface TransactionsRepository extends JpaRepository<Transactions, Integer> {
    ...
    Page<Transactions> findByStatusAndUserUsernameContaining(TransactionEnum status, String username,
            Pageable pageable);
    ...
    // Revenue grouped by ISO week label (e.g. "2024-W03") â€” last 12 weeks, SQL
    // Server native
    @Query(value = "SELECT CONCAT(CAST(YEAR(t.created_at) AS VARCHAR), '-W', RIGHT('0' + CAST(DATEPART(iso_week, t.created_at) AS VARCHAR), 2)), "
            +
            "       SUM(t.price) " +
            "FROM transactions t " +
            "WHERE t.created_at >= :since " +
            "GROUP BY YEAR(t.created_at), DATEPART(iso_week, t.created_at) " +
            "ORDER BY YEAR(t.created_at), DATEPART(iso_week, t.created_at)", nativeQuery = true)
    List<Object[]> findRevenueByWeek(@Param("since") LocalDateTime since);

    // Revenue grouped by month label (e.g. "2024-03") â€” last 12 months, SQL
    // Server native
    @Query(value = "SELECT CONCAT(CAST(YEAR(t.created_at) AS VARCHAR), '-', RIGHT('0' + CAST(MONTH(t.created_at) AS VARCHAR), 2)), "
            +
            "       SUM(t.price) " +
            "FROM transactions t " +
            "WHERE t.created_at >= :since " +
            "GROUP BY YEAR(t.created_at), MONTH(t.created_at) " +
            "ORDER BY YEAR(t.created_at), MONTH(t.created_at)", nativeQuery = true)
    ...
    @Query(value = "SELECT CAST(YEAR(t.created_at) AS VARCHAR), SUM(t.price) " +
            "FROM transactions t " +
            "GROUP BY YEAR(t.created_at) " +
            "ORDER BY YEAR(t.created_at)", nativeQuery = true)
